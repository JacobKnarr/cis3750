...
router.post('/upload',VerifyToken, type, function (req,res) {
  var dir = 'public/profilePic/';
...
if (!fs.existsSync(dir)){
  ...
  var target_path = dir + req.email +"_"+req.file.originalname;
  var dbPath = 'profilePic/' + req.email +"_"+req.file.originalname;
  ...
  src.on('end', function() {
    ...
    pg.connect(connectionString, function(err, client, done){
      ...
      var query = client.query('update users set picPath=($1) where email=($2)', [dbPath, req.email]);
      res.send('Image path updated');
      done();
    })
